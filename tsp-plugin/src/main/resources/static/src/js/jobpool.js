$(function () {
    ...
    $('#data-add').on('click', function (e) {
        layer.open({
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/jobPool/add.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        $('#data-query').click();
                    }
                    layer.close(index);
        ...
        if (checkedInputs.length > 0) {
            layer.alert('确定删除全部选中的数据吗?', {
                icon: 2, btn: ['确认', '取消'], yes: function (index) {
                    ...
                    form.push('</form>');
                    $.post(path + '/jobPool/delete.json', $(form.join('')).serialize(), function (result) {
                        if (result['success']) {
                            $('#data-query').click();
                        }
                    });
    ...
    $('.data-modify').on('click', function (e) {
        layer.open({
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/jobPool/update.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        layer.close(index);
                        $('#data-query').click();
                    }
